...
public class Board {
	 ...
	 * 
	 */
	public Integer utilityFunction(Move move, Player.PlayerID playerID) {
		...
		//if (ProbableMoves.contains(move)) {
			...
			if (startRow + 1 == endRow || startRow - 1 == endRow) {
				if(playerID.name().equalsIgnoreCase("BLACK")) {
					difference = Normal_BlackPcs - Normal_WhitePcs + King_BlackPcs *2- King_WhitePcs*2; 
				}
				else if(playerID.name().equalsIgnoreCase("WHITE")) {
					difference =  Normal_WhitePcs - Normal_BlackPcs + King_WhitePcs *2- King_BlackPcs*2; 
				}
	...
	public boolean isMovingOwnPiece(int row, int col, Player.PlayerID playerID) {
